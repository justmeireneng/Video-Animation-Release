...
document.querySelector('#profile').addEventListener('change',e=>{{const p=contract.profiles.find(x=>x.profile_id===e.target.value);if(!p)return;mode.value=p.mode;document.querySelector(`input[name="gender"][value="${{p.design?.gender||'male'}}"]`).checked=true;document.querySelector('#pitch').value=p.design?.pitch||'low';speed.value=p.speed;document.querySelector('#speed-label').textContent=Number(p.speed).toFixed(2)+'x';document.querySelector('#voice-id').value=p.voice_id||'';visibility();}});
</script></body></html>'''
