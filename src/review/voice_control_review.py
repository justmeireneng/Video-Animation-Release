...
def voice_control_page(control: VoiceControlService, message: str = "") -> str:
    ...
    age_control = (
        '<div id="age-wrap"><label>Age</label><select name="age" id="age">'
        f'<option {"selected" if state["age"]=="young adult" else ""}>young adult</option>'
        f'<option {"selected" if state["age"]=="middle-aged" else ""}>middle-aged</option>'
        f'<option {"selected" if state["age"]=="older adult" else ""}>older adult</option>'
        '</select></div>'
        if state["voiceMode"] == "voice_design" and caps.get("age") else ""
    )
    reference_control = (
        '<div id="reference-wrap"><label>Clone reference audio</label>'
        '<input type="file" name="reference_audio" accept="audio/*"></div>'
        if state["voiceMode"] == "voice_clone" and caps.get("reference_audio") else ""
    )
    cards = "".join(
...
.engine-name{{padding:12px;border:1px solid #49698d;border-radius:9px;background:#0f1b2d;font-weight:700}}.speed-presets{{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}}.speed-preset{{padding:8px;font-size:12px}}
...
<label>VOICE ENGINE</label><div class="engine-name">OmniVoice</div>
...
<label>Speed · <span id="speed-label">{float(state["speed"]):.2f}x</span></label><input type="range" name="speed" id="speed" min="0.85" max="1.20" step="0.01" value="{float(state["speed"]):.2f}"><div class="speed-presets"><button class="speed-preset" type="button" data-speed="0.95">0.95 Slow</button><button class="speed-preset" type="button" data-speed="1.00">1.00 Normal</button><button class="speed-preset" type="button" data-speed="1.08">1.08 Natural+</button><button class="speed-preset" type="button" data-speed="1.10">1.10 Default</button><button class="speed-preset" type="button" data-speed="1.12">1.12 Fast</button><button class="speed-preset" type="button" data-speed="1.15">1.15 Fast+</button></div>
...
document.querySelectorAll('.speed-preset').forEach(button=>button.addEventListener('click',()=>{{speed.value=button.dataset.speed;speed.dispatchEvent(new Event('input'));}}));
document.querySelector('#profile').addEventListener('change',e=>{{const p=contract.profiles.find(x=>x.profile_id===e.target.value);if(!p)return;mode.value=p.mode;document.querySelector(`input[name="gender"][value="${{p.design?.gender||'male'}}"]`).checked=true;document.querySelector('#pitch').value=p.design?.pitch||'moderate';speed.value=p.speed;document.querySelector('#speed-label').textContent=Number(p.speed).toFixed(2)+'x';document.querySelector('#voice-id').value=p.voice_id||'';visibility();}});
</script></body></html>'''
            ...
            try:
                if self.path == "/generate":
                    ...
                    control.update_state(
                        mode=values["mode"], gender=values.get("gender", "male"),
